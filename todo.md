...
## AMQP 消息接收问题诊断（2024-12-12）

- [ ] 测试完整的 Replay 流程（添加比赛 → 启动播放 → 检查状态）
- [ ] 检查 AMQP 监听器是否正在运行
- [ ] 检查 AMQP 连接配置（MQ 地址、访问令牌、路由键）
- [ ] 验证 Replay 服务器是否正在发送消息
- [ ] 检查 WebSocket 消息推送逻辑
- [ ] 修复消息接收和展示问题

## Node ID 支持修复（2024-12-12）

- [x] 修复后端 addToPlaylist 方法添加 node_id 参数支持
- [x] 修复前端添加比赛时传递 nodeId
- [x] 修复数据库保存 node_id 到会话
- [x] 验证 node_id 在 API 调用中正确传递
- [x] 重启服务器应用所有修改

## 部署错误修复（2024-12-12）

- [x] 删除 server/venv 虚拟环境目录
- [x] 更新 .gitignore 忽略虚拟环境目录
- [x] 保存新检查点
- [x] 项目现在可以正常部署

## UI 改进和修复（2024-12-12）

- [x] 修复回放列表不显示已添加的比赛（使用数据库作为备份）
- [x] 添加 node_id 必填验证（添加比赛前必须设置）
- [x] 在 UI 上明确提示用户需要先设置 node_id
- [x] 更新输入框占位符为正确的比赛 ID 格式
- [x] 测试完整的添加比赛流程

## 回放列表显示问题（2024-12-12）- 已修复 ✅

- [x] 检查数据库中是否成功保存了比赛记录
- [x] 检查 session 是否正确创建
- [x] 检查前端查询逻辑是否正确
- [x] 修复 session 创建和数据保存逻辑（修改 getActiveReplaySession 为获取最新 session）
- [x] 验证添加比赛后列表正确显示
- [x] 编写并通过播放列表显示功能测试
